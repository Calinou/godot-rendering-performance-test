...
func _input(event) -> void:
	...
	var directional_light := $"/root/Test/DirectionalLight" as DirectionalLight3D
	var omni_lights := $"/root/Test/OmniLights" as Node3D
	...
	if event.is_action_pressed("toggle_fxaa"):
		...
		fxaa_status.text = "FXAA: Enabled" if get_viewport().screen_space_aa else "FXAA: Disabled"
